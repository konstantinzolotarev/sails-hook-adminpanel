...
module.exports = {
    ...
    processRequest: function(req, fields, cb) {
        ...
        _.forIn(postParams, function(val, key) {
            ...
            if (field.model.type == 'array' && _.isString(postParams[key]) && postParams[key].indexOf('[') === -1 && postParams[key].indexOf('{') === -1) {
                postParams[key] = postParams[key].split(',');
            }
            if (field.model.type == 'array' && !_.isArray(postParams[key]) && !_.isPlainObject(postParams[key]) && _.isPlainObject(JSON.parse(postParams[key])[0])) {
